...
set(CM0_SDK_SHA256
    ...
    CACHE STRING "Expected SHA-256 for the pinned CM0/CardputerZero BSP archive"
)
...
if(NOT EXISTS "${CM0_SDK_ROOT}/usr/include" OR
   NOT EXISTS "${CM0_SDK_ROOT}/usr/lib" OR
   NOT EXISTS "${CM0_SDK_ROOT}/usr/include/${CM0_MULTIARCH}/bits/wordsize.h" OR
   NOT EXISTS "${CM0_SDK_ROOT}/lib/${CM0_MULTIARCH}/ld-linux-aarch64.so.1")
    message(FATAL_ERROR
        "CM0 SDK sysroot appears incomplete: ${CM0_SDK_ROOT}\n"
        "Expected Debian aarch64 headers, libraries, and dynamic loader after "
        "extracting the pinned sdk_bsp.tar.gz."
    )
endif()
...
execute_process(
    COMMAND "${_CP0_C_COMPILER}" -dumpmachine
    OUTPUT_VARIABLE _CP0_COMPILER_TARGET
    OUTPUT_STRIP_TRAILING_WHITESPACE
    RESULT_VARIABLE _CP0_COMPILER_TARGET_RESULT
)
if(NOT _CP0_COMPILER_TARGET_RESULT EQUAL 0 OR
   NOT _CP0_COMPILER_TARGET MATCHES "^aarch64(-unknown)?-linux-gnu$")
    message(FATAL_ERROR
        "Cross compiler has unexpected target '${_CP0_COMPILER_TARGET}': "
        "${_CP0_C_COMPILER}"
    )
endif()
...
# GCC cross toolchains on macOS do not automatically add Debian's multiarch
# include directory when given an external sysroot. Keep this target-only path
# explicit; never fall back to host headers.
set(_CP0_MULTIARCH_INCLUDE_DIR "${CMAKE_SYSROOT}/usr/include/${CM0_MULTIARCH}")
list(APPEND CMAKE_C_STANDARD_INCLUDE_DIRECTORIES "${_CP0_MULTIARCH_INCLUDE_DIR}")
list(APPEND CMAKE_CXX_STANDARD_INCLUDE_DIRECTORIES "${_CP0_MULTIARCH_INCLUDE_DIR}")
...
set(CMAKE_C_STANDARD_INCLUDE_DIRECTORIES
    ...
    CACHE STRING "Target-only standard C include directories" FORCE
)
